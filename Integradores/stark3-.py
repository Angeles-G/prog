...
def extraer_iniciales(nombre_heroe:str)->str:
    ...
    if nombre_heroe != "":
        ...
        if mayusculas.count("THE")>0:
            nombre = re.sub("THE", " ", mayusculas)
...
def definir_iniciales_nombre(heroe:dict)->bool:
    '''
    Brief: defino una nueva key en el diccionario ingresado donde guardo las iniciales, obtenidas de la funcion 
            Extraer_iniciales. 
    Parameters:
...
def agregar_codigo_heroe(heroe:dict, id_heroe:int)->bool:
    '''
    Brief: genera una nueva key en el diccionario que recibe, con un value que sera el id generado en la funcion 
            generar_codigo_heroe.
    Parameters:
            id_heroe: numero entero. Se espera un numero del 1 al len de la lista de superheroes.
    ...
    if len(heroe) > 0:
        ...
        if id !=0:
            #M-00000001
            id_numerico = re.findall("([M&F&NB])([-])([0-9]{7,8})", id)
            if id_numerico != []:
    ...
    if num_str != " " and num_str != "":
        ...
        if tipo == str:
            ...
            if caracteres:
                ...
                cantidad_de_carcteres = len(caracter_sin_espacios)
                letras_encontradas = re.findall("[A-Za-z]+",num_str )
                numeros_encontrados = re.findall("[0-9]+",num_str)
                guiones_encontrados = re.findall("[-]+",num_str)
                if letras_encontradas != []:#si encontro letras entra
                    validar = -3
                elif numeros_encontrados != []: 
                    if guiones_encontrados == []:# si no encontro guiones entra
                        if len(numeros_encontrados) == 1:
                            if len(numeros_encontrados[0]) == cantidad_de_carcteres:
                                convertir = int(caracter_sin_espacios)
                                validar = convertir
                            else:
                                validar = -3
                        else:
                            validar = -3
                    elif len(guiones_encontrados) == 1:
                        if len(numeros_encontrados[0]) == (cantidad_de_carcteres -1):
                            indice = caracter_sin_espacios.index("-")
                            if indice == 0:
                                validar = -2
                            else:
                                validar = -3
    ...
    if num_str != " " and num_str != "":
        ...
        if tipo == str:
            ...
            if caracteres:
                ...
                numeros_encontrados = re.findall("[0-9]+",num_str)
                guiones_encontrados = re.findall("[-]+",num_str)
                puntos_encontrados = re.findall("[.]+",num_str)
                if letras_encontradas != []:
                    validar = -3
                elif numeros_encontrados != []:
                    cantidad_de_numeros = len(numeros_encontrados[0])+ len(numeros_encontrados[1])
                    if guiones_encontrados == []:
                        if len(numeros_encontrados) == 2:
                            if len(puntos_encontrados) == 1 and len(puntos_encontrados[0]) == 1:
                                convertir = float(caracter_sin_espacios)
                                validar = convertir
                            else:
                                validar = -3
                        else:
                        ...
                        if len(puntos_encontrados) == 1 and len(puntos_encontrados[0]) == 1:
                            if cantidad_de_numeros == (cantidad_de_carcteres -2):
                                indice_guion = caracter_sin_espacios.index("-")
                                indice_punto = caracter_sin_espacios.index(".")
                                if indice_guion == 0 and indice_punto > 1:
                                    validar = -2
                                else:
                                    validar = -3
                            else:
...
def sanitizar_string(valor_str: str, valor_por_defecto: str = "-")->bool:
    ...
    if (valor_str == " " or valor_str == "") and valor_por_defecto != "-":
        ...
        if caracter_sin_espacios.isalpha():
            ...
            numeros_encontrados = re.findall("[0-9]+", caracter_sin_espacios)
            if numeros_encontrados != []:#encontro numeros
                sanitizar = False
            elif caracter_sin_espacios.count("/") > 0:
                partir = re.split("/", caracter_sin_espacios)
                letra  = 0
                for parte in partir:
                    cambio = re.sub(" ", "a", parte)
                    if cambio.isalpha():
                        letra += 1
                if letra == len(partir):
                    caracter_sin_espacios = re.sub("/", " ", caracter_sin_espacios)
                    convertir = caracter_sin_espacios.lower()
                    sanitizar = True
                else:
                    sanitizar = False
                ...
                if valor_str == " " or valor_str == "":
                    ...
                    convertir_sin_espacios = re.sub(" ", "a", caracter_sin_espacios)
